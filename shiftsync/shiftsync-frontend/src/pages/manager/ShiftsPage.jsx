...
export function ShiftsPage() {
  ...
  const handleSaveShift = async (e) => {
    e.preventDefault();
    if (!shiftData.date || !shiftData.startTime || !shiftData.employeeId || !shiftData.role) return;
        ...
        <form onSubmit={handleSaveShift} className="space-y-6">
          <div className="grid grid-cols-2 gap-5">
            ...
            <div className="col-span-2 flex flex-col">
              <label className="mb-1.5 block text-sm font-semibold text-slate-700 uppercase tracking-tighter text-[11px]">Role Label</label>
              <select 
                className="flex h-11 w-full rounded-xl border border-slate-200/80 bg-white px-4 py-2 text-sm text-slate-900 focus:border-primary-500 focus:outline-none focus:ring-4 focus:ring-primary-500/10 transition-all shadow-sm"
